            ...
            <x-table.row wire:key="task-{{ $task->id }}">
                <x-table.td>
                    <div>
                        ...
                        <div class="text-truncate" style="font-size: 0.7rem; color: #94a3b8; max-width: 250px;">
                ...
                <x-table.td>
                    <div class="d-flex align-items-center gap-2 text-nowrap">
                        ...
                        <span
                            style="font-size: 0.8125rem; color: #475569; line-height: 1;">{{ $task->assignee->name ?? 'Unassigned' }}</span>
                    </div>
                </x-table.td>
                <x-table.td>
                    @php
                    ...
                    <span class="d-flex align-items-center gap-1"
                        style="font-size: 0.75rem; font-weight: 600; color: {{ $pColor }};">
                ...
                <x-table.td>
                    <span
                        style="font-size: 0.8125rem; color: #475569;">{{ \Carbon\Carbon::parse($task->due_date)->format('M d, Y') }}</span>
                </x-table.td>
                <x-table.td>
                    @php
                    ...
                    @if ($scope === 'my')
                        ...
                        <div class="dropdown" wire:key="status-{{ $task->id }}">
                            <button
                                class="btn badge rounded-pill d-flex align-items-center gap-1 border-0 dropdown-toggle"
                            ...
                            <ul class="dropdown-menu shadow-sm border-0"
                                ...
                                @foreach ($allStatuses as $s)
                                    <li><button class="dropdown-item py-2"
                                            wire:click="updateStatus({{ $task->id }}, {{ $s->id }})">
                                            <i class="bi bi-circle-fill me-2"
                                                style="color: {{ $s->color }}; font-size: 0.5rem;"></i>
                                            {{ $s->name }}
                                        </button></li>
                                @endforeach
                            </ul>
